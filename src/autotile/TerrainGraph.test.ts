...
describe("isValidAdjacency", () => {
	...
	it("allows DeepWater ↔ ShallowWater", () => {
		expect(isValidAdjacency(BiomeId.DeepWater, BiomeId.ShallowWater)).toBe(
			true,
		);
		expect(isValidAdjacency(BiomeId.ShallowWater, BiomeId.DeepWater)).toBe(
			true,
		);
	});
	...
	it("allows ShallowWater ↔ land biomes", () => {
		...
		expect(
			isValidAdjacency(BiomeId.ShallowWater, BiomeId.DenseForest),
		).toBe(true);
	});
	...
	it("rejects DeepWater ↔ land (needs ShallowWater buffer)", () => {
		...
		expect(isValidAdjacency(BiomeId.DeepWater, BiomeId.DenseForest)).toBe(
			false,
		);
	});
});
...
describe("getValidFallback", () => {
	it("returns ShallowWater when neighbor is DeepWater", () => {
		expect(getValidFallback(BiomeId.Grass, BiomeId.DeepWater)).toBe(
			BiomeId.ShallowWater,
		);
		expect(getValidFallback(BiomeId.Sand, BiomeId.DeepWater)).toBe(
			BiomeId.ShallowWater,
		);
	});

	it("returns Grass for other invalid adjacencies", () => {
		expect(getValidFallback(BiomeId.DeepWater, BiomeId.Grass)).toBe(
			BiomeId.Grass,
		);
	});
});
...
describe("deriveTerrainFromCorners", () => {
	it("returns the biome when all 4 corners are the same", () => {
		...
		expect(
			deriveTerrainFromCorners(DeepWater, DeepWater, DeepWater, DeepWater),
		).toBe(DeepWater);
	});
	...
	it("breaks 2v2 ties by higher priority", () => {
		...
		expect(
			deriveTerrainFromCorners(ShallowWater, ShallowWater, Grass, Grass),
		).toBe(Grass);
		expect(
			deriveTerrainFromCorners(Grass, ShallowWater, Grass, ShallowWater),
		).toBe(Grass);
	});
	...
	it("handles 4-way tie (1 each) by highest priority", () => {
		...
		expect(
			deriveTerrainFromCorners(DeepWater, ShallowWater, Sand, Grass),
		).toBe(Grass);
	});
	...
	it("handles 3 distinct biomes", () => {
		...
		expect(
			deriveTerrainFromCorners(Grass, Grass, ShallowWater, Sand),
		).toBe(Grass);
	});
});
