...
export class EditorPanel {
	...
	constructor() {
		...
		this.brushSizeButton.addEventListener("click", () =>
			this.cycleBrushSize(),
		);
		...
		this.bridgeButton.addEventListener("click", () =>
			this.cycleBridgeDepth(),
		);
		...
		modeSep.style.cssText =
			"width: 1px; height: 32px; background: #555; margin: 0 4px;";
	...
	toggleMode(): void {
		...
		this.modeButton.textContent =
			this.brushMode === "tile" ? "Tile" : "Grid";
		this.modeButton.style.borderColor =
			this.brushMode === "subgrid" ? "#f0a030" : "#888";
	}
	...
	private updateBrushSizeButton(): void {
		...
		this.brushSizeButton.style.borderColor =
			this.brushSize > 1 ? "#a4f" : "#888";
	}

	private updateBridgeButton(): void {
		const label =
			this.bridgeDepth === 0 ? "B:Off" : `B:${this.bridgeDepth}`;
		this.bridgeButton.textContent = label;
		this.bridgeButton.style.borderColor =
			this.bridgeDepth > 0 ? "#4a9" : "#888";
	}
		...
		for (let i = 0; i < PALETTE.length; i++) {
			...
			if (btn && entry) {
				btn.style.borderColor =
					entry.terrainId === this.selectedTerrain ? "#fff" : "#555";
				btn.style.boxShadow =
					entry.terrainId === this.selectedTerrain
