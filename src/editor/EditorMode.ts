...
export class EditorMode {
	...
	private screenToSubgrid(
		sx: number,
		sy: number,
	): { gsx: number; gsy: number } {
		const { wx, wy } = this.camera.screenToWorld(sx, sy);
	...
	private paintTileAt(sx: number, sy: number): void {
		...
		if (tx === this.lastPaintedTile.tx && ty === this.lastPaintedTile.ty)
			return;
	...
	private paintSubgridAt(sx: number, sy: number): void {
		...
		if (
			gsx === this.lastPaintedSubgrid.gsx &&
			gsy === this.lastPaintedSubgrid.gsy
		)
			return;
	...
	private updatePinch(): void {
		...
		if (this.pinchStartDist > 0) {
			...
			this.camera.zoom = Math.max(
				0.05,
				Math.min(3, this.pinchStartZoom * ratio),
			);
		}
