...
describe("BiomeMapper.classify", () => {
  ...
  it("maps mid-low elevation to ShallowWater", () => {
    ...
    expect(BiomeMapper.classify(0.41, 0.5)).toBe(BiomeId.ShallowWater);
  });
  ...
  it("boundary: elevation exactly at threshold", () => {
    ...
    expect(BiomeMapper.classify(0.3, 0.5)).toBe(BiomeId.ShallowWater);
    // elevation 0.42, moisture 0.4 → Grass (>= 0.42 passes ShallowWater)
    expect(BiomeMapper.classify(0.42, 0.4)).toBe(BiomeId.Grass);
  });

  it("grass blends directly into water (no beach band)", () => {
    // Elevation just above shallow water threshold → land biome
    expect(BiomeMapper.classify(0.42, 0.4)).toBe(BiomeId.Grass);
    // Elevation just below → shallow water
    expect(BiomeMapper.classify(0.41, 0.4)).toBe(BiomeId.ShallowWater);
  });
});
